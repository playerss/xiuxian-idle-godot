...
func _assert_break_eta_tip() -> void:
	...
	await get_tree().process_frame
# 打磨-150: 境界阶梯 层内 进度行 (打磨-34 _ladder_prog) / 道行阶段 进度行 (打磨-35 _dao_prog) tooltip 动态段
# (打磨-142/144/145/146/147/148/149 已给 突破 CTA/下一目标行/速率行/离线收益行/灵石内联行/进度行/ETA 行
# 补 动态段, 但 境界阶梯 两条 金色 进度行 悬停 只有 静态 口径 说明, 挂机 扫视 悬停 不知 攒满 本层/本阶段
# 还要 挂 多久 (速率/成功率/期望 失败 重烧 成本) — 单源 复用 GameData.break_eta_tip [与 ETA 行/CTA 同 表达式
# 防 漂移, 两行 共用 同一 动态段 文案, 口径 随 飞升 翻转], 静态 前缀 存 成员 防 split 坑,
# _refresh 动态段 文本 变化 才 刷 [同 打磨-142/148/149 缓存 口径, 挂机 恒定 无 每帧 重建]; 纯展示 无 存档/统计 副作用)
# 断言 (手动驱动 确定性): 双行 节点 存在/两行 静态前缀 含 口径 说明+动态段 标记/初始 tooltip = 静态前缀+接口 恒等
# x2/动态段 速率+成功率+ETA+期望 齐全/同态 节流 冻结窗口 无 副作用/层11 跨档 1分 同步/境界2 速率 15+77%+2分 同步
# /攒够 切 已攒够 无段/飞升 道行 口径 两行 同 刷新 窗口 (层内 行 文本 空串 隐藏+道行 行 文本 动态 段 同源)/收尾 复原 干净 基准
func _assert_ladder_prog_tip() -> void:
	var g := GameData
	var lp: Label = ui._ladder_prog
	var dp: Label = ui._dao_prog
	check(lp != null and dp != null, "打磨-150 境界阶梯 层内/道行 进度行 节点 存在")
	check(str(ui._ladder_prog_tip_static).find("【突破 明细 (动态)】") >= 0
			and str(ui._ladder_prog_tip_static).find("当前 速率") >= 0
			and str(ui._dao_prog_tip_static).find("【道行 精进 明细 (动态)】") >= 0
			and str(ui._dao_prog_tip_static).find("当前 速率") >= 0,
			"打磨-150 两行 静态前缀 含 口径 说明 + 动态段 标记")
	...
	ui._refresh()
	var dyn0: String = g.break_eta_tip()
	var t0: String = str(lp.tooltip_text)
	var u0: String = str(dp.tooltip_text)
	check(t0 == str(ui._ladder_prog_tip_static) + dyn0
			and u0 == str(ui._dao_prog_tip_static) + dyn0,
			"打磨-150 初始 两行 tooltip = 静态前缀+接口 恒等 (实际 %s)" % t0.left(40))
	check(dyn0.begins_with("当前 %s 灵气/秒" % g.fmt(g.qi_per_sec()))
			and dyn0.find("突破成功率 85%") >= 0 and dyn0.find("突破还需 不足1分") >= 0
			and dyn0.find("· 期望次数") >= 0 and dyn0.find("· 期望总消耗") >= 0,
			"打磨-150 动态段 速率+成功率+ETA+期望 齐全 (实际 %s)" % dyn0.left(60))
	# 同态 节流: 冻结 UI 窗口 内 两行 tooltip 稳定 无重写, 无 资源/统计/境界 副作用
	var snap: Dictionary = g.stats.duplicate(true)
	var ess0: float = g.essence
	var st0: float = g.stones
	var gproc: bool = g.is_processing()
	...
	await get_tree().process_frame
	ui._refresh()
	check(str(lp.tooltip_text) == t0 and str(dp.tooltip_text) == u0
			and str(ui._ladder_prog_tip) == dyn0 and str(ui._dao_prog_tip) == dyn0
			and g.stats == snap and g.essence == ess0 and g.stones == st0,
			"打磨-150 同态 节流 两行 tooltip 稳定 无 资源/统计 副作用 (窗口内 冻结 UI)")
	# 层 11 → 消耗 x11 缺口 110, 速率 1.0 → ETA 1分 档 (不足1分→1分 跨档 真 变化, 两行 同 窗口 同步;
	# 行 文本 走 _refresh_ladder_eta 1 秒 节流 路径, 手动驱动 后 强制 刷 防 节流 窗口 未 跨 1 秒 漏 刷 踩 坑)
	g.layer = 11
	ui._refresh()
	ui._refresh_ladder_eta(true)
	var t1: String = str(lp.tooltip_text)
	check(t1 == str(ui._ladder_prog_tip_static) + g.break_eta_tip()
			and t1.find("突破还需 1分") >= 0 and t1 != t0
			and str(dp.tooltip_text) == str(ui._dao_prog_tip_static) + g.break_eta_tip()
			and str(dp.tooltip_text) != u0
			and lp.text.begins_with("第 11/9 层"),
			"打磨-150 层11 跨档 ETA 1分 两行 动态 同步 + 行 文本 同源 (实际 %s)" % g.break_eta_tip().left(40))
	# 境界2 层20 → 速率 15 段 + 成功率 77% 段 + ETA 2分 档 动态 同步 (层内 行 文本 第 20/3 层 同源 窗口)
	g.realm_idx = 2
	g.layer = 20
	ui._refresh()
	ui._refresh_ladder_eta(true)
	var t2: String = str(lp.tooltip_text)
	check(t2 == str(ui._ladder_prog_tip_static) + g.break_eta_tip()
			and t2.begins_with(str(ui._ladder_prog_tip_static) + "当前 15 灵气/秒")
			and t2.find("突破成功率 77%") >= 0 and t2.find("突破还需 2分") >= 0
			and lp.text.begins_with("第 20/3 层"),
			"打磨-150 境界2 速率/成功率/ETA 段 两行 动态 同步 = 接口 恒等 (实际 %s)" % g.break_eta_tip().left(40))
	g.realm_idx = 0
	g.layer = 1
	# 攒够 → 动态段 切 已攒够 无 ETA/成功率/期望 段 (两行 同 窗口, 行 文本 保持 当前 已攒 口径)
	g.essence = g.breakthrough_cost() + 1.0
	ui._refresh()
	var d3: String = str(lp.tooltip_text).substr(str(ui._ladder_prog_tip_static).length())
	var e3: String = str(dp.tooltip_text).substr(str(ui._dao_prog_tip_static).length())
	check(d3.find("已攒够, 点击突破/修炼") >= 0 and d3.find("突破还需") < 0 and d3.find("成功率") < 0
			and d3.find("期望次数") < 0
			and e3 == d3
			and str(lp.tooltip_text) == str(ui._ladder_prog_tip_static) + g.break_eta_tip(),
			"打磨-150 攒够 两行 动态段 切 已攒够 = 接口 恒等 (实际 %s)" % d3.left(50))
	# 飞升 道行 口径 — 两行 同 刷新 窗口: 层内 行 文本 空串 隐藏 + 道行 行 文本 动态 段 同源
	g.ascended = true
	g.dao_level = 0
	g.dao = 5.0e8
	ui._refresh()
	ui._refresh_ladder_eta(true)
	var t4: String = str(dp.tooltip_text)
	check(t4 == str(ui._dao_prog_tip_static) + g.break_eta_tip()
			and t4.begins_with(str(ui._dao_prog_tip_static) + "当前 1 道行/秒")
			and t4.find("道行精进成功率") >= 0 and t4.find("道行精进还需") >= 0
			and lp.text == "" and dp.text.find("道行精进需") >= 0,
			"打磨-150 飞升 道行 口径 两行 同 刷新 窗口 (层内 行 隐藏+道行 行 文本 动态 段 同源) (实际 %s)" % t4.left(50))
	# 收尾: 恢复 前序 段 态 + 挂机 进程 + 干净 基准
	g.essence = es
	g.learned.clear()
	for x in ln:
	...
	ui.set_process(uiproc)
	ui._refresh()
	check(str(lp.tooltip_text) == str(ui._ladder_prog_tip_static) + g.break_eta_tip()
			and str(dp.tooltip_text) == str(ui._dao_prog_tip_static) + g.break_eta_tip(),
...
# M8-3 打磨-155b: 技能页 120 行 5 类别 字形 图标 接入 断言 — 120 行 图标 挂 徽章 右侧 child1
# (16px 纯 装饰 无 热区 垂直 居中) / 类别 = 数据 接口 恒等 / 5 类 全覆盖 / 24 主动 金 爆发 描边 档
# + 96 被动 无 描边 (接口 单源) / tooltip = skill_detail 接口 恒等 含 类别字形 口径 行 /
# 同态 再刷 幂等 不 重绘 节点 不 重建 无 统计 副作用 / 收尾 恢复 干净 基准
func _assert_m155b_skill_icons() -> void:
	var g := GameData
	g.set_process(false)
	ui._tab.current_tab = 1
	ui._refresh()
	...
	check(ui._skill_icon_nodes.size() == g.skill_ids.size(),
			"M8-3 打磨-155b 技能 图标 节点 全量 (实际 %d/%d)" % [ui._skill_icon_nodes.size(), g.skill_ids.size()])
	var cat_seen155: Dictionary = {}
	var act_cnt155 := 0
	var icfail155 := ""
	for id in g.skill_ids:
		var s: Dictionary = g.skill_by_id[id]
		...
		if ic == null or ic.get_parent() != hb or badge.get_parent() != hb:
			icfail155 = id + " 缺 图标"
			break
		var cat: String = str(s["category"])
		var active: bool = g.skill_is_active(id)
		check(ic.mouse_filter == Control.MOUSE_FILTER_IGNORE
					and ic.size_flags_vertical == Control.SIZE_SHRINK_CENTER
					and str(ic.custom_minimum_size) == str(Vector2(16, 16)),
					"M8-3 打磨-155b 技能 %s 图标 纯 装饰 无 热区 + 垂直 居中 + 16px 口径 (实际 %s)" % [id, str(ic.custom_minimum_size)])
		check(ic.get_category() == cat and ic.is_active_marked() == active,
					"M8-3 打磨-155b 技能 %s 图标 类别 = 数据 %s + 主动档 = 接口 (实际 %s/%s)" % [id, cat, str(ic.get_category()), str(ic.is_active_marked())])
		cat_seen155[cat] = true
		if active:
			act_cnt155 += 1
	check(icfail155 == "", "M8-3 打磨-155b 120 行 图标 挂 徽章 右侧 全覆盖 (首个 失 %s)" % icfail155)
	check(cat_seen155.size() == 5,
			"M8-3 打磨-155b 5 类别 全覆盖 (实际 %d: %s)" % [cat_seen155.size(), str(cat_seen155.keys())])
	check(act_cnt155 == 24, "M8-3 打磨-155b 24 主动 描边 档 计数 (实际 %d)" % act_cnt155)
	# tooltip = skill_detail 接口 恒等 + 含 类别字形 口径 行
	var trow155: Node = ui._skill_row_nodes[g.skill_ids[0]]
	var tid155: String = g.skill_ids[0]
	check(str(trow155.tooltip_text) == g.skill_detail(tid155)
				and str(trow155.tooltip_text).find(g.skill_cat_glyph_tip(tid155)) >= 0,
				"M8-3 打磨-155b tooltip = skill_detail 恒等 且 含 类别字形 口径 行 (实际 %s)" % str(trow155.tooltip_text).get_slice("\n", 3))
	# 挂机 恒定: 同态 再刷 幂等 不 重绘 节点 不 重建 无 统计 副作用
	var sic155: Control = ui._skill_icon_nodes[tid155]
	var rc155: int = sic155.redraw_count
	var snap155: Dictionary = g.stats.duplicate(true)
	ui._refresh()
	await get_tree().process_frame
	check(ui._skill_icon_nodes.size() == g.skill_ids.size() and sic155.redraw_count == rc155 and g.stats == snap155,
			"M8-3 打磨-155b 同态 再刷 幂等 不 重绘 节点 不 重建 无 统计 副作用 (实际 %d)" % sic155.redraw_count)
	# 收尾: 恢复 干净 基准 (纯 UI 节点 构建 一次 无 状态 变更, 仅 复位 页面/进程)
	ui._tab.current_tab = 3
	ui._refresh()
	await get_tree().process_frame
