...
func _build_tower_page(page: Panel) -> void:
	...
	_build_tower_card(cards, "endless", "登天梯", "无尽 塔 · 无 层数 上限 · 每 100 层 里程碑 Boss + 宝箱 (保底 稀有+ 词缀) · 500 层后 全部 魔化 · 每日首胜 额外 灵石")
...
func _refresh_tower() -> void:
	...
	var key: String = "%d|%d|%s|%s|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d" % [
		...
		g._auto_tower_losses,  # 打磨-128: 会话 败局 累计 入键 (0 胜局 全败 帧 wins 不 变 层数 不 变, 键 须 感知 losses 变化 才 刷 卡层 段 — 同 打磨-122 漏入键 真 bug 防 漏刷)
		g._auto_tower_new_records,  # 打磨-132: 会话 新纪录 次数 入键 (手动 测试/外部 置态 不 升层 也 须 刷 会话 段; 真实 路径 新纪录 必 伴随 胜局 wins 天然 感知)
		g._auto_tower_milestones,  # 打磨-133: 会话 里程碑 宝箱 次数 入键 (手动 测试/外部 置态 不 升层 也 须 刷 会话 段; 真实 路径 宝箱 必 伴随 胜局 wins 天然 感知)
...
func _apply_tower_card(tid: String, floor_n: int, floor_txt: String, is_clear: bool, ratio: float, mon: Dictionary, win: bool) -> void:
	...
	var def_txt: String = g.tower_power_def_line(float(mon["def"]))
	if str(def_l2.text) != def_txt:
		def_l2.text = def_txt
...
func _refresh() -> void:
	...
	# 打磨-79: 顶栏 灵石速率 常显 (与 修行页 灵石速率 行 同 口径, 文本 变化 才刷; 速率<=0 隐藏)
	var sr_t: String = g.stone_rate_text()
	if sr_t != _sr_text:
		_sr_text = sr_t
		_sr_label.text = sr_t
	...
	# 打磨-80: 自动领悟开关 按钮态 (开关状态 变化才刷; 读档恢复/外部改 同步)
	if g.auto_learn != _auto_learn_on:
...
# 打磨-80: 自动领悟开关 — 点击切 开/关 (存档持久化, 由 GameData._process 驱动 境界解锁 后 自动 批量 领悟);
# 底部消息确认口径 (开关动作 本身 无 学习/资源 副作用, 实际 领悟 由 _try_auto_learn 走 learn_all_available 口径)
func _on_auto_learn() -> void:
	GameData.auto_learn = not GameData.auto_learn
	_auto_learn_on = GameData.auto_learn
