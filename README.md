...
挂机自动积累 **灵气** 与 **灵石**，灵气攒够后点击突破境界（有成功率）；开启 **自动突破** 后攒够即自动突破/精进（打磨-67），开启 **自动购置** 后灵石攒够即自动购买法器/装备并最佳换装（打磨-68），开启 **自动施展** 后主动神通冷却完毕即自动爆发（打磨-69），修行页底部一行 **自动系列状态汇总**（✓=开/✗=关，打磨-70），各段可点击切换对应开关（打磨-71），顶栏 **一键系列状态汇总徽标** 六段可执行数一览（打磨-75）+ 各段悬浮明细（打磨-76），顶栏 **挂机时长** 常显（⏳ X小时Y分，打磨-77）+ 顶栏 **主资源速率** 常显（+X/秒，未飞升=灵气 飞升后=道行，打磨-78），挂机全程无需手动点击。
