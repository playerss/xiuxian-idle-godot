...
- [ ] 打磨-37（候选）：成功率构成 tooltip（成功率行 tooltip 现只有静态说明, 玩家不知 +N% 来自哪; GameData.primary_break_chance_parts() 拆 基础(境界/阶段)/功法/装备/钳制 四段数值, main.gd 成功率行 tooltip 动态展示构成, 数值变化才刷; 自测补断言）
