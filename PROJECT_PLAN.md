...
- [ ] 打磨-51（候选）：法器区属性构成 tooltip（10 件法器行只显示 boost 倍率, 玩家不知灵气速率具体怎么叠 (基础x境界x法器连乘x功法装备); 法器行 tooltip 追加 当前贡献/购买后预览 两行数值对比, 复用 GameData 只读接口, 构建时静态+购买后刷; 顺带 一键购买 浮动文案可加 灵气速率 +N% 变化量）
