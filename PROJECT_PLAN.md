...
- [ ] 打磨-54（候选）：主动神通行 爆发预览（技能页 24 个主动神通行只显示 冷却倒计时/爆发秒数, 玩家点 施展 前不知能爆发多少灵气(道行); 行内/tooltip 加 '爆发 +N 灵气 (当前速率 x 爆发秒数)' 预览, 飞升后口径=道行, 与 打磨-51 法器属性构成 口径对齐 (只读预览, 速率/境界/飞升状态 变化才刷, 1 秒节流); GameData 复用 qi_per_sec + 技能 value, ui_test/selftest 补断言）
