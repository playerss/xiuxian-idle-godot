...
- [ ] 打磨-35（候选）：当前道行阶段消耗提示（飞升后当前道行行只有金色高亮+下方 ETA, 与打磨-34 未飞升层内进度口径不齐; 飞升后当前道行阶段行下加金色 "道行精进需 X 道行 (当前 X)" 行 (道祖封顶行 "已至道祖, 道法自然 ♪"), 标签随道行阶段 move_child, 未飞升隐藏; GameData.dao_progress_text() 复用 dao_break_cost/fmt, 自测补断言）
