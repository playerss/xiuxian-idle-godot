...
- [ ] 打磨-49（候选）：顶栏灵石速率 + 可购进度 — 顶栏灵石行只有总量, 玩家不知攒 1 件最便宜未拥有装备/法器 还要挂多久; 灵石行 tooltip 或 修行页 加 "当前 X 灵石/秒, 距下一件「X」约 Y 可购" (复用打磨-12 stone_per_sec + 打磨-47/48 next_target + eta 口径); 仅修行页/顶栏 文本刷新, 无新存档字段
