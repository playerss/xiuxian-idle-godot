...
- [ ] 打磨-42（候选）：成就页收集进度一览 mini 进度条（打磨-28 的 技能 N/120 · 装备 N/140 · 法器 N/10 · 成就 N/17 目前是纯文字汇总, 收集控玩家扫视时难以快速判断哪类快集齐; 现把 _collect_label 单行文字升级为 4 条 (类别名+计数 + mini 进度条) 横排: 每类一条 6px 高 ColorRect 进度条按 已收集/总量 填充 (复用打磨-40 进度条模式, 金/青 二态: 满=金, 未满=青), 比例变化 1% 量化档才刷 (防每帧重绘); 需 GameData.collect_summary() 已提供 已收集/总量, UI 侧新增 4 个 {bar_bg, bar_fill, label} 节点 + 比例缓存, ui_test 补断言 4 条进度条节点/初始 0 填充/满态金色/半态比例, headless 0 报错回归)
