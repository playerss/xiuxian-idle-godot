...
- [ ] 打磨-55（候选）：单个主动神通 施展 浮动反馈统一（单个 神通行 点 施展 目前只走底部 _show_msg 一行 "施展「X」! 瞬间获得灵气 N!", 与 打磨-45 一键系列 屏幕中央绿色浮动 口径不齐 (单个施展 是核心点击反馈, 批量才有浮动); 单个施展 成功时也弹 绿色浮动 "✦ 施展「X」 ✦" (数量恒 1 与 一键施展 文案口径区分, 位置/颜色/上浮淡出 与 一键系列 同 _onekey_float 口径复用), 失败 (未领悟/冷却中) 不弹; 无存档/统计副作用 (skill_use 埋点口径不变); selftest/ui_test 补断言 (ui_test 受控态 领悟 1 个神通 点 施展 按钮: 浮动计数+1/文案含 神通名/绿色/底部消息仍在/冷却中再点 0 浮动/未领悟 不弹))
