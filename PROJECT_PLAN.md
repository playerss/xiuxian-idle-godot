...
- [ ] 打磨-38（候选）：技能页"只看可学"筛选（120 技能含大量境界不足项, 玩家找"现在能学的"成本高; 技能页品质筛选行尾加 "只看可学" 开关按钮 (与 类别/品质 筛选 AND 叠加, 过滤掉 境界/层 不足的技能, 已学技能仍显示在最前; 按钮随当前可学数变化文案, 关闭态显示 "显示全部"); GameData.skill_can_learn_display(id) 复用 can_learn 口径, main.gd _on_learnable_filter 切换 + _apply_skill_filter 内应用 + 按钮文本仅变化才刷; 自测补断言）
