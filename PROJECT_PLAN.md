...
- [ ] 打磨-45（候选）：一键系列按钮统一浮动反馈 — 一键领悟/一键购买/一键最佳/一键施展 的批量结果目前只走底部 _show_msg, 批量数量大时反馈弱且易被后续消息覆盖; 四个一键按钮执行变更 >0 时屏幕中央绿色浮动提示上浮淡出 (复用 _float 系列 tween 口径, 文案含数量 如 "✦ 一键领悟 12 个技能 ✦"), 0 变更只走底部消息不变; 无存档/统计副作用, 浮动次数计数器供自测断言
