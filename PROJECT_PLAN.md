...
- [ ] 打磨-57（候选）：主动神通 就绪 浮动提示（已学主动神通 冷却完毕 时玩家需自己扫 冷却倒计时 才知道可施展, 挂机时缺"就绪"反馈; 复用 打磨-17 成就解锁 浮动 口径, 某神通 冷却结束转就绪 时 屏幕中央 绿色浮动 "✦「X」冷却完毕 ✦" (文案含神通名, 与 一键施展/单个施展 浮动 口径区分), 同一批多个就绪 合并一行, 只读 _active_cd 不改动状态/存档/统计)
