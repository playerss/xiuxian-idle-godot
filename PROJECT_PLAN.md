...
- [ ] 打磨-48（候选）：下一件缺口行的 ETA 联动 — 打磨-47 的 "距下一件「X」还差 Y 灵石" 只有数量没有时间预期, 玩家不知要挂多久; 灵石速率 >0 时缺口行追加 "约 X 可购" (复用打磨-12 eta_seconds/eta_text 口径), 无灵石收入时省略; GameData.eta_text 已支持, 仅需 UI 文案拼接 + selftest/ui_test 补断言, 无新存档字段
