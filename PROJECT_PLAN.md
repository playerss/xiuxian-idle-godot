...
- [ ] 打磨-41（候选）：技能/装备/法器行品质色标（260 行数据现仅靠 名称/文案 区分品质, 扫描时 凡~神 七档难辨; 每行行首加品质色竖条 4px (复用 GameData.TIER_COLOR, 技能 tier 0..5 / 装备 tier 0..6 / 法器无品质按境界档着色或统一青色), 状态高亮金框 (打磨-1) 保持不变, ui_test 补断言 色标存在+颜色与数据 tier 一致)
