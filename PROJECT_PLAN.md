...
- [ ] 打磨-50（候选）：修行页灵石速率行内联 下一件可购 ETA — 打磨-49 的可购进度只在 顶栏灵石行 tooltip (需悬停才见), 挂机扫视时修行页看不到; 修行页 灵石速率行 (_stone_rate_label) 右侧/下方追加内联 "距下一件「X」约 Y 可购" 金色小字 (灵石足够隐藏/全拥有 已集齐 提示), 复用 stone_next_target_tip 口径或拆出 stone_next_target_eta_inline 接口; 仅文本变化才刷, 无新存档字段
